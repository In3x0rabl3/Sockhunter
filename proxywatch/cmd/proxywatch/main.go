...
func runScan(minScore int, roleFilter map[string]bool) []candidate.Candidate {
...
func main() {
	...
	printSummary := func(cands []candidate.Candidate, killScore int, doKill bool) {
		if len(cands) == 0 {
		...
		for _, c := range cands {
			name := candidate.TrimName(c.Proc.Name, 18)
	...
	printDetails := func(cands []candidate.Candidate) {
		for _, c := range cands {
			fmt.Println()
			fmt.Printf("[PID %d] %s\n", c.Proc.Pid, c.Proc.Name)
			...
			if (c.Role == "reverse-control" || c.Role == "reverse-transport") && c.ControlChannel != nil {
				scope := "external"
				if candidate.IsInternalIP(c.ControlChannel.RemoteAddress) {
					scope = "internal"
				}
